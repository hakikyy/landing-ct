...
const certConfig = {
  bnsp: { badge: 'BNSP Certified' },
  kemnaker: { badge: 'Kemnaker Certified' },
  internal: { badge: 'Training Certificate' },
};

const catInfo = {
  k3: { method: 'In-Class / In-House', provider: 'LSP K3', categoryLabel: 'K3 & Keselamatan Kerja' },
  pjk3: { method: 'In-Class', provider: 'Kemnaker RI', categoryLabel: 'Pembinaan K3' },
  environmental: { method: 'In-Class / In-House', provider: 'LSP Lingkungan', categoryLabel: 'Lingkungan Hidup' },
  ...
  food: { method: 'In-Class / In-House', provider: 'LSP Pangan', categoryLabel: 'Keamanan Pangan' },
  sdm: { method: 'In-Class / In-House', provider: 'LSP SDM', categoryLabel: 'SDM & Soft Skills' },
};
...
// Read existing backup and merge
const existing = JSON.parse(fs.readFileSync('src/data/services/bnsp.json.bak', 'utf-8'));
const merged = {
  _meta: existing._meta,
  services: [...services, ...existing.services]
};

fs.writeFileSync('src/data/services/bnsp.json', JSON.stringify(merged, null, 2));
console.log(`✓ Generated ${services.length} BNSP services`);
console.log(`✓ Total services in bnsp.json: ${merged.services.length}`);
console.log(`✓ File saved to src/data/services/bnsp.json`);
